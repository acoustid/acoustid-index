...
class Options {
 public:
    ...
    Options(const QHash<QString, QString> &options, const QList<QString> &arguments) : m_options(options), m_arguments(arguments) {}

    bool contains(const QString &name) const { return m_options.contains(name); }

    QString option(const QString &name) const { return m_options.value(name); }

    int argumentCount() const { return m_arguments.size(); }

    const QString &argument(int i) const { return m_arguments.at(i); }

    const QList<QString> &arguments() const { return m_arguments; }

    void addArgument(const QString &argument) { m_arguments.append(argument); }

    void addOption(const QString &name, const QString &value) { m_options.insert(name, value); }
...
/*
        ...
        parser.addOption("file", 'f').setArgument().setHelp("input
   file").setMetaVar("INPUT"); parser.addOption("test", 't'); parser.parse(argc,
   argv);
*/

...
Option::Option(const QString &longName) : m_longName(longName), m_shortName(0), m_argument(NoArgument) { m_metaVar = m_longName.toUpper(); }
...
OptionParser::OptionParser(const QString &usage) : m_usage(usage) { addOption("help").setShortName('h').setHelp("show this help message and exit"); }

OptionParser::~OptionParser() { qDeleteAll(m_options); }
...
QString OptionParser::generateUsage() { return QString("Usage: ") + m_usage.replace("%prog", m_prog); }
...
void OptionParser::exit(int code) { ::exit(code); }

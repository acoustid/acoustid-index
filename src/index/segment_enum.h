...
class SegmentEnum {
 public:
    SegmentEnum(SegmentIndexSharedPtr index, SegmentDataReader *dataReader) : m_index(index), m_dataReader(dataReader), m_block(0), m_currentBlock(nullptr) {}
    ...
    uint32_t key() { return m_currentBlock->key(); }

    uint32_t value() { return m_currentBlock->value(); }

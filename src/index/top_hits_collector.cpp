...
TopHitsCollector::TopHitsCollector(size_t numHits, int topScorePercent) : m_numHits(numHits), m_topScorePercent(topScorePercent) {}

TopHitsCollector::~TopHitsCollector() {}

void TopHitsCollector::collect(uint32_t id) { m_counts[id] = m_counts[id] + 1; }

struct CompareByCount {
    CompareByCount(const QHash<uint32_t, unsigned int> &counts) : m_counts(counts) {}
    bool operator()(uint32_t a, uint32_t b) { return m_counts[a] > m_counts[b]; }

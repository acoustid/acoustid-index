...
class VersionedDocId {
 public:
    VersionedDocId(uint32_t docId, uint32_t version, bool isDeleted) : m_docId(docId), m_version(version), m_isDeleted(isDeleted) {}
    ...
    bool operator==(const VersionedDocId &other) const { return m_docId == other.m_docId && m_version == other.m_version && m_isDeleted == other.m_isDeleted; }

    bool operator!=(const VersionedDocId &other) const { return !(*this == other); }
...
class SegmentDocsIterator {
 public:
    SegmentDocsIterator(std::map<uint32_t, std::pair<uint32_t, bool>>::const_iterator it) : m_it(it) {}
    ...
    SegmentDocsIterator &operator++() {
        ++m_it;
        return *this;
    }
    SegmentDocsIterator operator++(int) {
        SegmentDocsIterator it = *this;
        ++m_it;
        return it;
    }
...
class SegmentDocs {
 public:
    ...
    const_iterator begin() const { return SegmentDocsIterator(m_docs.begin()); }

    const_iterator end() const { return SegmentDocsIterator(m_docs.end()); }

...
class IndexInfoData : public QSharedData {
 public:
    ...
    IndexInfoData(const IndexInfoData& other)
        : QSharedData(other), segments(other.segments), nextSegmentNum(other.nextSegmentNum), revision(other.revision), attribs(other.attribs) {}
    ~IndexInfoData() {}
...
class IndexInfo {
 public:
    ...
    int revision() const { return d->revision; }

    // Set the index revision
    void setRevision(int revision) { d->revision = revision; }

    // Increment the index revision by one and return it
    int incRevision() { return ++d->revision; }

    size_t segmentCount() const { return d->segments.size(); }

    const SegmentInfo& segment(int idx) const { return d->segments.at(idx); }

    const SegmentInfoList& segments() const { return d->segments; }

    SegmentInfoList& segments() { return d->segments; }

    void setSegments(const SegmentInfoList& segments) { d->segments = segments; }

    void clearSegments() { d->segments.clear(); }

    void addSegment(const SegmentInfo& info) { d->segments.append(info); }

    size_t lastSegmentId() const { return d->nextSegmentNum; }

    size_t incLastSegmentId() { return ++d->nextSegmentNum; }

    void setLastSegmentId(size_t n) { d->nextSegmentNum = n; }

    const QMap<QString, QString>& attributes() const { return d->attribs; }

    QMap<QString, QString>& attributes() { return d->attribs; }

    bool hasAttribute(const QString& name) const { return d->attribs.contains(name); }

    QString attribute(const QString& name) const { return d->attribs.value(name); }

    void setAttribute(const QString& name, const QString& value) { d->attribs.insert(name, value); }

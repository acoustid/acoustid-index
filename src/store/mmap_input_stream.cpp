...
MMapInputStream::MMapInputStream(const FSFileSharedPtr &file) : MemoryInputStream(file->mmapAddress(), file->mmapLength()), m_file(file) {}

int MMapInputStream::fileDescriptor() const { return m_file->fileDescriptor(); }

const FSFileSharedPtr &MMapInputStream::file() const { return m_file; }

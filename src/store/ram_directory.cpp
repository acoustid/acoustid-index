...
RAMDirectory::~RAMDirectory() { qDeleteAll(m_data.values()); }

void RAMDirectory::close() {}

QStringList RAMDirectory::listFiles() { return m_data.keys(); }

bool RAMDirectory::fileExists(const QString &name) { return m_data.contains(name); }
...
void RAMDirectory::renameFile(const QString &oldName, const QString &newName) { m_data.insert(newName, m_data.take(oldName)); }
...
const QByteArray &RAMDirectory::fileData(const QString &name) { return *m_data.value(name); }

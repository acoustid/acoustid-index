...
FSOutputStream::FSOutputStream(const FSFileSharedPtr &file) : m_file(file) {}

FSOutputStream::~FSOutputStream() {
    flushBuffer();  // XXX this indirectly calls a virtual function write()
}

int FSOutputStream::fileDescriptor() const { return m_file->fileDescriptor(); }
...
QString NamedFSOutputStream::fileName() const { return m_fileName; }

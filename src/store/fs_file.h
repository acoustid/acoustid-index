...
class FSFile {
 public:
    explicit FSFile(int fd, void* addr = NULL, size_t length = 0) : m_fd(fd), m_addr(addr), m_length(length) {}
    ...
    const uint8_t* mmapAddress() const { return static_cast<const uint8_t*>(m_addr); }

    size_t mmapLength() const { return m_length; }

...
MemoryInputStream::MemoryInputStream(const uint8_t *addr, size_t length) : m_addr(addr), m_length(length), m_position(0) {}

MemoryInputStream::~MemoryInputStream() {}

size_t MemoryInputStream::position() { return m_position; }

void MemoryInputStream::seek(size_t position) { m_position = std::min(position, m_length); }

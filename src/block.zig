...
// Minimal fixed-capacity buffer replacing std.BoundedArray (removed in Zig 0.16).
// Only the surface used by BlockEncoder below: buffer/len fields, slice(),
// unusedCapacitySlice(), clear().
fn BoundedArray(comptime T: type, comptime capacity: usize) type {
    return struct {
        ...
        fn unusedCapacitySlice(self: *Self) []T {
...
test "BlockReader basic functionality" {
    ...
    var block_data: [256]u8 align(4) = undefined;
    ...
    try testing.expectEqual(4, reader.getNumItems());
    try testing.expectEqual(100, reader.getMinHash());
...
test "BlockReader range-based docid decoding" {
    ...
    var block_data: [512]u8 align(4) = undefined;
    const items_consumed = try encoder.encodeBlock(&items, min_doc_id, &block_data);
    ...
    const range100 = reader.findHash(100);
    try testing.expectEqual(0, range100.start);
    try testing.expectEqual(3, range100.end);

    const docids100 = reader.getDocidsForRange(range100);
    try testing.expectEqual(3, docids100.len);
    try testing.expectEqual(1001, docids100[0]);
    try testing.expectEqual(1005, docids100[1]);
    try testing.expectEqual(1010, docids100[2]);

    // Test another small range
    const range200 = reader.findHash(200);
    try testing.expectEqual(3, range200.start);
    try testing.expectEqual(5, range200.end);

    const docids200 = reader.getDocidsForRange(range200);
    try testing.expectEqual(2, docids200.len);
    try testing.expectEqual(2001, docids200[0]);
    try testing.expectEqual(2002, docids200[1]);

    // Test range at end
    const range300 = reader.findHash(300);
    try testing.expectEqual(5, range300.start);
    try testing.expectEqual(8, range300.end);

    const docids300 = reader.getDocidsForRange(range300);
    try testing.expectEqual(3, docids300.len);
    try testing.expectEqual(3001, docids300[0]);
    try testing.expectEqual(3002, docids300[1]);
    try testing.expectEqual(3003, docids300[2]);
}
...
pub const BlockEncoder = struct {
    ...
    out_hashes: BoundedArray(u8, MAX_BLOCK_SIZE) = .{},
    out_hashes_control: BoundedArray(u8, MAX_BLOCK_SIZE) = .{},

    out_docids: BoundedArray(u8, MAX_BLOCK_SIZE) = .{},
    out_docids_control: BoundedArray(u8, MAX_BLOCK_SIZE) = .{},
    ...
    pub fn encodeBlock(self: *Self, items: []const Item, min_doc_id: u32, out: []u8) !usize {
        ...
        const bytes_written = writer.buffered().len;
...
test "BlockEncoder with mixed hashes and docids" {
    ...
    try testing.expectEqual(5, reader.getNumItems());
    try testing.expectEqual(1, reader.getMinHash());

    // Test hash ranges
    const range1 = reader.findHash(1);
    try testing.expectEqual(0, range1.start);
    try testing.expectEqual(2, range1.end);

    const range3 = reader.findHash(3);
    try testing.expectEqual(2, range3.start);
    try testing.expectEqual(3, range3.end);

    const range4 = reader.findHash(4);
    try testing.expectEqual(3, range4.start);
    try testing.expectEqual(4, range4.end);

    const range5 = reader.findHash(5);
    try testing.expectEqual(4, range5.start);
    try testing.expectEqual(5, range5.end);
...
test "BlockEncoder with duplicate hashes" {
    ...
    const header = decodeBlockHeader(&block);
    try testing.expectEqual(100, header.min_hash);
    try testing.expectEqual(3, header.num_items);
    ...
    try testing.expectEqual(3, num_items);

    // All items should have hash 100 and docids 1, 2, 3
    for (items_output, 0..) |item, i| {
        try testing.expectEqual(100, item.hash);
...
test "BlockEncoder reuse across multiple blocks" {
    ...
    const min_doc_id: u32 = 1;
    var block1: [256]u8 align(4) = undefined;
    ...
    var block2: [256]u8 align(4) = undefined;

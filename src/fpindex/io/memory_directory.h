...
class MemoryDirectory : public Directory {
 public:
    ...
    std::shared_ptr<Database> OpenDatabase(const std::string& name, bool create = false) override;
    std::vector<std::string> ListFiles() override;
    std::vector<std::string> ListDirectories() override;

 private:
    std::mutex mutex_;
    std::map<std::string, std::variant<std::shared_ptr<MemoryFile>, std::shared_ptr<MemoryDirectory>, std::shared_ptr<Database>>> entries_;
};

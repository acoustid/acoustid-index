...
Handler::Handler(QSharedPointer<Session> session, const QString &name, const QStringList& args)
	: m_session(session), m_name(name), m_args(args)
{
}
...
void Handler::run()
{
	...
	m_session->metrics()->onRequest(m_name, elapsedSeconds(timer));
}
